...
sVals = svdvals(xClean) #get largest singular value for regularization parameter
ridgeNormal = ridgeNumber*singularScale*sVals[0]

crossVal = sklearn.model_selection.KFold(n_splits=cvNum)
folds = [next(crossVal.split(featureMat)) for i in range(cvNum)] #do this to pick one particular split for cv
